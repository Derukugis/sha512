...
fn sha512(s: &str) -> [u8; 64]{
    ...
    let total_len = ((bytes.len() + 17 + 127) / 128) * 128;
    let mut m = vec![0u8; total_len];
    ...
    m[total_len - 16..].copy_from_slice(&bit_len.to_be_bytes());
...
fn main() {
    let hash_bytes = sha512("hello");
    
    // convert raw bytes to hex string
    let hash_hex: String = hash_bytes.iter()
        .map(|b| format!("{:02x}", b))
        .collect();
